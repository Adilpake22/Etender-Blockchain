...
class Blockchain {
    ...
    public static function createBidHash(float $amount, string $secret): string {
        return hash('sha256', $amount . '|' . $secret);
    }

    // Verify bid hash matches original commitment
    public static function verifyBidHash(float $amount, string $secret, string $storedHash): bool {
        return hash_equals($storedHash, self::createBidHash($amount, $secret));
    }

    // Generate a random secret key for bidders
